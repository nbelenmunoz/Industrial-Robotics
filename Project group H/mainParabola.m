...
m2 = 3.15;
m3 = 0.433;
payload_initial = 0;  % Initial payload mass
m4_initial = m1 + m2 + m3 + payload_initial;
m = [m1; m2; m3; payload_initial];
...
% Inertia matrix 
M_initial = diag([payload_initial, payload_initial, payload_initial, m3, m3, m3, ...
                 (Jg1 + Jg2 + Jg3), m2, m2, m2, (Jg1 + Jg2), ...
                 m1, m1, m1, Jg1, m4_initial]);

%External force vector
Fse_initial = zeros(16, 1);  
Fse_initial(16) = -(m4_initial) * g_const;

%% Definition of waypoints for trajectory
Waypoints1 = [
...
%% Define Payload Cases
payload_cases = [0, 0.2, 0.5, 0.8, 1]; % in kg
num_payloads = length(payload_cases);
Fq_all = cell(num_payloads,1); % To store torques for each payload
colors = lines(num_payloads);  % Generate distinguishable colors
prismatic_conversion = 49.846 / 1000; % Nm/N


%% Forces and Torques calculation with dynamic payloads
for p_idx = 1:num_payloads
    current_payload_case = payload_cases(p_idx);
    fprintf('Calculating torques for payload: %.1f kg\n', current_payload_case);
    
    Fq_current = zeros(length(time_vec), 4);
    ...
    for k = 1:length(time_vec)
        ...
        % Get the payload for the current segment and payload case
        current_payload_segment = getPayloadForSegment(seg_idx, current_payload_case);
        m4 = m1 + m2 + m3 + current_payload_segment;
        
        % Update Inertia Matrix M
        M = diag([current_payload_segment, current_payload_segment, current_payload_segment, ...
                  m3, m3, m3, (Jg1 + Jg2 + Jg3), m2, m2, m2, ...
                  (Jg1 + Jg2), m1, m1, m1, Jg1, m4]);
    
        % Update External Force Vector Fse
        Fse = zeros(16, 1);  
        Fse(16) = -(m4) * g_const;
    ...
    % Store the torques for the current payload case
    Fq_all{p_idx} = Fq_current;
end

% Display torque ranges for each payload
for p_idx = 1:num_payloads
    fprintf('Payload: %.1f kg\n', payload_cases(p_idx));
    max_Fq = max(Fq_all{p_idx});
    min_Fq = min(Fq_all{p_idx});
    fprintf('  Maximum Torques (Nm): %.2f, %.2f, %.2f, %.2f\n', max_Fq);
    fprintf('  Minimum Torques (Nm): %.2f, %.2f, %.2f, %.2f\n', min_Fq);
end
...
rectangle('Position', [homePositionE(1)-0.1, homePositionE(2)-0.05, 0.1, 0.1], ...
          'FaceColor', [0.8 0.8 1], 'EdgeColor', 'b');
rectangle('Position', [positionD(1)-0.2, positionD(2)-0.125, 0.2, 0.25], ...
          'FaceColor', [0.8 0.8 1], 'EdgeColor', 'b');

% Draw object base A, B, C, D
rectangle('Position', [positionA(1)-0.075, positionA(2)-0.075, 0.15, 0.15], ...
          'FaceColor', [0.8 0.8 1], 'EdgeColor', 'k');
rectangle('Position', [positionB(1)-0.075, positionB(2)-0.075, 0.15, 0.15], ...
          'FaceColor', [0.8 0.8 1], 'EdgeColor', 'k');
rectangle('Position', [positionC(1)-0.075, positionC(2)-0.075, 0.15, 0.15], ...
          'FaceColor', [0.8 0.8 1], 'EdgeColor', 'k');
rectangle('Position', [positionD(1)-0.15, positionD(2)-0.075, 0.15, 0.15], ...
          'FaceColor', [0.8 0.8 1], 'EdgeColor', 'k');
...
hold on;
for p_idx = 1:num_payloads
    plot(time_vec, Fq_all{p_idx}(:, 1), 'Color', colors(p_idx,:), 'DisplayName', sprintf('%.1f kg', payload_cases(p_idx)));
end
...
hold on;
for p_idx = 1:num_payloads
    plot(time_vec, Fq_all{p_idx}(:, 2), 'Color', colors(p_idx,:), 'DisplayName', sprintf('%.1f kg', payload_cases(p_idx)));
end
...
hold on;
for p_idx = 1:num_payloads
    plot(time_vec, Fq_all{p_idx}(:, 3), 'Color', colors(p_idx,:), 'DisplayName', sprintf('%.1f kg', payload_cases(p_idx)));
end
...
% Force on Prismatic Joint (with conversion)
subplot(4,1,4);
hold on;
for p_idx = 1:num_payloads
    plot(time_vec, Fq_all{p_idx}(:, 4) * prismatic_conversion, 'Color', colors(p_idx,:), 'DisplayName', sprintf('%.1f kg', payload_cases(p_idx)));
end
...
function payload = getPayloadForSegment(seg_idx, payload_case)

    if (seg_idx >= 6 && seg_idx <= 11) || (seg_idx >= 19 && seg_idx <= 24) ||(seg_idx >= 32 && seg_idx <= 37)
